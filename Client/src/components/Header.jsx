...
export default function Header() {
    return (
        <header className='bg-rose-400 shadow-md fixed top-0 left-0 w-full z-50'>
            ...
            {currentUser && currentUser.isAdmin && (<Link to='/manageBooks'><li className='hidden sm:inline text-white hover:underline'>Manage books</li></Link>)}
            {currentUser && currentUser.isAdmin && (<Link to='/manageReviews'><li className='hidden sm:inline text-white hover:underline'>Manage reviews</li></Link>)} */}
                    <Link to='/login'>
